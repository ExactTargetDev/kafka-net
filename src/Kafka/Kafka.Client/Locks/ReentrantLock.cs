...
namespace Kafka.Client.Locks
{
    ...
    public class ReentrantLock : ILock, IDisposable, ConditionVariable.IExclusiveLock
    {
        ...
        private abstract class Sync
        {
            ...
            {
                get { lock (this) return this._owner; }
            }

            public int HoldCount
            {
                get { lock (this) return this.IsHeldByCurrentThread ? this._holds : 0; }
            }

            public bool IsHeldByCurrentThread
            {
                get { lock (this) return Thread.CurrentThread == this._owner; }
            }

            public bool IsLocked
            {
                get { lock (this) return this._owner != null; }
            }
        ...
        private sealed class NonfairSync : Sync
        {
            ...
            public override void LockInterruptibly()
            {
                ...
                lock (this)
                {
                    ...
                    {
                        if (this._owner == null) Monitor.Pulse(this);
                        throw e;
                    }
                }
            }
            ...
            public override bool TryLock(TimeSpan durationToWait)
            {
                ...
                lock (this)
                {
                    if (this.GetHold(caller)) return true;
        ...
        private sealed class FairSync : Sync, IQueuedSync, IDeserializationCallback
        {
            ...
            private WaitNode GetSignallee(Thread caller)
            {
                lock (this)
                {
                    ...
                    if (this._holds >= 2) // current thread will keep the lock
                    {
                        --this._holds;
                        return null;
                    }
        ...
        {
            get { return this.sync.HoldCount; }
        ...
        {
            get { return this.sync.IsHeldByCurrentThread; }
        ...
        public virtual int QueueLength
        {
            get { return this.sync.QueueLength; }
        ...
        {
            this.sync = fair ? (Sync)new FairSync() : new NonfairSync();
        }
        ...
        {
            return this.sync.TryLock(timeSpan);
        }
        ...
        {
            return this.IsFair ? new FIFOConditionVariable(this) : new ConditionVariable(this);
        }
        ...
        {
            get { return this.sync.HasQueuedThreads; }
        }
        ...
        public virtual ICollection<Thread> QueuedThreads
        {
            get { return this.sync.QueuedThreads; }
        }
        ...
        {
            return this.AsConditionVariable(condition).HasWaiters;
        }
        ...
        {
            return this.AsConditionVariable(condition).WaitQueueLength;
        }
        ...
        {
            return this.AsConditionVariable(condition).WaitingThreads;
        }
